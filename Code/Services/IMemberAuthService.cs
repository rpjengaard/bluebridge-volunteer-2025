...
public interface IMemberAuthService
{
    ...
    Task LogoutAsync();
    Task<SignupResult> SignupAsync(string email, string password, string firstName, string lastName, string? phone, DateTime? birthdate, string? zipcode, List<int>? crewWishes, string? memberWish, List<string>? selectedTimeslots);
